...
# turn it grayscale, blur it, get its edges, create a kernel to dilate and thin it out, the result
def pre_processing(img):
    img_gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    img_blur = cv2.GaussianBlur(img_gray, (5, 5), 1)
    img_canny = cv2.Canny(img_blur, 200, 200) # canny alleviates shadows in fonts
    kernel = np.ones((5, 5))
...
while True:
    ...
    biggest = getContours(img_pre_processed)
    if biggest.size != 0:
        imgWarped = getWarp(img, biggest)
        imageArray = ([img,img_pre_processed], [imgContour,imgWarped])
        # imageArray = ([imgContour, imgWarped])
        cv2.imshow("ImageWarped", imgWarped)
    else:
        # imageArray = ([img, img_pre_processed],
        #               [img, img])
